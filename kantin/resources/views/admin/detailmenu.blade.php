    ...
    <div class="mt-5">
        ...
        <a href="/admin/daftarmenu/{{ $menus->id }}" class="{{ request()->is('admin/daftarmenu/'.$menus->id) ? 'transition-shadow duration-500 cursor-pointer flex flex-row mt-5 border bg-emerald-100 rounded-lg hover:shadow' : 'transition-shadow duration-500 cursor-pointer flex flex-row mt-5 border rounded-lg hover:shadow' }}">
            ...
            <div class="text-xs ml-3 mt-3">
                <p class="font-bold capitalize">{{ $menus->nama_menu}}</p>
                <p class="flex items-center mt-4"><img src="{{ asset('images/icons/brand.png') }}" alt="Ikon" class="w-3 mr-2">Rp {{ number_format($menus->harga, 2, ',', '.') }}</p>
                <p class="flex items-center my-2"><img src="{{ asset('images/icons/box-open.png') }}" alt="Ikon" class="w-3 mr-2">{{ $menus->stock}}</p>
                <p class="flex items-center my-2 capitalize"><img src="{{ asset('images/icons/user.png') }}" alt="Ikon" class="w-3 mr-2">{{ $menus->user->nama}}</p>
            </div>
        </a>
        @endforeach
    </div>
...
<div class="bg-white ml-[310px] h-screen p-5 rounded-lg">
    ...
    <div class="p-5">
            ...
            <div class="text-xs mt-10">
                <p class="font-bold text-xl capitalize">{{ $menu->nama_menu}}</p>
                <p class="flex items-center mt-4"><img src="{{ asset('images/icons/brand.png') }}" alt="Ikon" class="bg-white w-6 p-1 rounded-full">Rp {{ number_format($menu->harga, 2, ',', '.') }}</p>
                <p class="flex items-center my-2"><img src="{{ asset('images/icons/box-open.png') }}" alt="Ikon" class="bg-white w-6 p-1 rounded-full">{{ $menu->stock}}</p>
                <p class="flex items-center my-2 capitalize"><img src="{{ asset('images/icons/user.png') }}" alt="Ikon" class="bg-white w-6 p-1 rounded-full">{{ $menu->user->nama}}</p>
