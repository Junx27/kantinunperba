...
<div class="fixed top-2 text-xs w-[300px] h-[760px] bg-white p-5 rounded-lg mr-3 overflow-auto">
    ...
    <div class="mt-5">
        ...
        <a href="/admin/pesananmasuk/{{ $pembayaran->id }}" class="{{ $pembayaran->id == $pesanan_terendah->id ? 'transition-all duration-500 my-2 p-4 bg-lime-400 rounded-lg cursor-pointer flex flex-col' : 'transition-all duration-500 my-2 p-4 hover:bg-lime-400 hover:rounded-lg cursor-pointer flex flex-col' }}">
        ...
        <div class="text-xs w-full h-full rounded-lg">
            ...
            <h1 class="text-start my-5 font-bold ml-2 capitalize">Detail pesanan:</h1>
                ...
                <div class="flex justify-end mt-5">
                    <form action="/admin/pesananmasuk/{{ $pesanan_terendah->id }}" method="post" onclick="return confirm('Konfirmasi pengiriman')">
                        @method('put')
                        @csrf
