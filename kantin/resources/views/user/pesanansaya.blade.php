...
<div class="fixed top-2 text-xs w-[300px] h-[760px] bg-white p-5 rounded-lg mr-3 overflow-auto">
    ...
    <div class="mt-5">
        ...
        @if ($pembayaran->metode_pembayaran == 'Pilih Metode Pembayaran')
        <a href="/user/pembayaran/{{ $pembayaran->id }}" class="{{ $pembayaran->id == $pesanan_terendah->id ? 'transition-all duration-500 my-2 p-4 bg-lime-400 rounded-lg cursor-pointer flex flex-col' : 'transition-all duration-500 my-2 p-4 hover:bg-lime-400 hover:rounded-lg cursor-pointer flex flex-col' }}">
            ...
            <div class="flex justify-between items-center">
                ...
                <form action="/user/pembayaran/{{ $pembayaran->id }}" method="post" onclick="return confirm('Apakah Anda yakin ingin menghapus data ini?')">
                    @method('delete')
                    @csrf
    ...
    <div class="z-10 sticky top-0 mt-2 text-xs w-full h-[110px] bg-white p-5 rounded-lg">
        ...
        <div class="text-xs w-full h-full rounded-lg">
            ...
            <h1 class="text-start my-5 font-bold ml-2 capitalize">Detail pesanan:</h1>
                ...
                <a href="/user/pembayaran/{{$pesanan_terendah->id}}/edit" class="mt-10 flex justify-end">
                    <p class="bg-lime-400 p-2 rounded ">Pesan Sekarang </p>
                </a>
            </div>
        </div>
</div>
